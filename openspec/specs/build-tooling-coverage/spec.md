# build-tooling-coverage Specification

## Purpose

Defines the Vitest coverage provider, its dev dependency, and the report shape that `npm run coverage` produces for the `svelte-adapter-azure-swa` root package. The capability pins the provider to V8 (via `@vitest/coverage-v8`, kept at the same major as `vitest`), preserves the existing coverage output directory (`./coverage-test`), the reporter set (`text`, `html`, `clover`, `json`, `lcov`), and the exclude list, so consumers of the generated reports see no behavioral change beyond the provider swap.
