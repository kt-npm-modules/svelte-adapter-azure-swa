# adapter-authorization-policy Specification

## Purpose

Defines the adapter's policy and request-mutation contract for the inbound `Authorization` header on Azure Static Web Apps managed Functions. The capability covers the `preserveAuthorization` option, the strip-by-default behaviour, the case-insensitive matching rule, the unchanged `x-ms-original-url` consumption, and the typed nested `x-adapter-test-workarounds` JSON contract (`emptyFormContentTypeStrip` + `auth` namespaces) that lets tests observe the pre-strip Authorization comparison without exposing raw secrets.

The default-strip behaviour is motivated by empirical evidence (issue #218) that real Azure SWA routinely overwrites or injects `Authorization` before the request reaches the managed Function on every adapter-supported HTTP method. The escape hatch (`preserveAuthorization: true`) preserves the previous behaviour for consumers who explicitly want the platform-supplied value.
