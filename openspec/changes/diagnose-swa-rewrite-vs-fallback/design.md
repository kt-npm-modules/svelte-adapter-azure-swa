...
### Decision 5: Test updates — minimal, structural only

The existing test file [tests/demo/e2e/diagnostic-headers.test.ts](tests/demo/e2e/diagnostic-headers.test.ts) is updated, not redesigned. Concretely:

1. Replace the file-scope `const PROBE_PATH = '/diagnostic-headers'` with a `ROUTE_MODES` object mapping `'nav-fallback'` and `'rewrite'` to their full URL paths, plus a `RouteMode` type alias.
2. Thread `routeMode: RouteMode` through the existing `runAuthProbe`/`runForwardedProbe` helper option types and through `fetchWithMethod`. The helpers already accept an options object — adding one more field is a one-line change in each helper signature.
3. The `attachFacts` call site uses `${routeMode}/${probeKey}.json` instead of `diagnostic-headers/${probeKey}.json`. No change to attachment content; no change to the safety string-search guard in `assertCoreShape`.
4. The annotation pushed by `runAuthProbe` includes the route mode in its description (e.g. `nav-fallback GET get-auth: preserved`), so a reader of the Playwright report sees both modes per method without parsing JSON.
5. Wrap the existing seven auth probes in a `for (const routeMode of ROUTE_MODES_LIST)` loop — or duplicate the `test.describe` block twice with the route mode parameterized via `test.describe.parallel.each` / a small wrapper. Either yields fourteen auth probes (two per method).
6. The existing `get-baseline-no-auth` probe runs once per route mode (two baselines total). The `get-baseline-no-auth-repeat` and `get-spoof-forwarded` probes are removed — see "Pruned forwarded probes" below.

Helper signatures (`controlHeaders`, `freshControls`, `getFacts`, `decodeFactValue`, `kebabToCamel`, `attachFacts`, `classifyAuthorization`, `assertCoreShape`, `resolveOrigin`) are unchanged — only their callers (`runAuthProbe`, `runForwardedProbe`) thread the new `routeMode` field through to the path lookup and the attachment name. The four assertions inside `assertCoreShape` and the safety string-search guard are unchanged.

**Alternatives considered:** redesign the test as two separate test files, one per mode — rejected, the user constraint asks us not to redesign the tests; a per-mode loop reuses 100% of the existing helpers and keeps the test count predictable. Parameterize via Playwright projects — rejected, would require editing `playwright.config.ts` and changes how the local `npm run test:swa` runs work; out of scope.
